...
export function render_markdown(md_text, result_html_div = null, markedOptions = {}) {
    /*
    ...
    function convert_js_markdown_variables(markdown_text, use_placeholder) {
        ...
        const patterns_and_replacements = [
            ...
            },
            {
                ...
                replacement:
                    '<iframe style="width: 100%;aspect-ratio: 16 / 9;" src="https://www.youtube.com/embed/$1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>',
    ...
    renderer.code = (text, params = "") => {
        ...
        return `${filename ? filenameEl : ""}<pre class="code-block"><code data-filename="${filename}" class="${language ? `language-${language}` : ""
...
function render_problem_text(md_id, html_id, placeholder = false) {
    ...
    // Replace occurrences of &gt; at the beginning of a new line with ">"
    // so Markdown blockquotes are handled correctly
    const md_text = md_div.html().trim();
